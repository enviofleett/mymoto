...
const supabaseKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImNtdnBuc3FpZWZic3Frd25yYWthIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc2NzcyMjAwMSwiZXhwIjoyMDgzMjk4MDAxfQ.d5LxnXgAPC7icY_4nzxmmANz4drZ3dX7lnr97XNoFVs";
    ...
    if (i > 0) {
      ...
      if (dist > 0.1) {
          ...
          if (positions && positions.length > 0) {
              console.log(`  Points in gap:`);
              positions.forEach(p => {
                  console.log(`    ${p.gps_time}: (${p.latitude}, ${p.longitude}) Speed:${p.speed} Ign:${p.ignition_on}`);
              });
          }
      }
    }
