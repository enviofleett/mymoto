...
const sheetVariants = cva(
  // Premium Glass Sheet Style
  cn(
    "fixed z-50 gap-4 p-6 transition ease-in-out",
    // Glass effect
    "bg-[rgba(24,24,27,0.9)] backdrop-blur-[20px] backdrop-saturate-[180%]",
  ...
  ({ side = "right", className, children, ...props }, ref) => (
    <SheetPortal>
      ...
      <SheetPrimitive.Content ref={ref} className={cn(sheetVariants({ side }), className)} {...props}>
        {children}
        <SheetPrimitive.Close className="absolute right-4 top-4 rounded-lg p-1 opacity-70 ring-offset-background transition-all hover:opacity-100 hover:bg-[rgba(255,255,255,0.1)] focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none">
          <X className="h-4 w-4" />
