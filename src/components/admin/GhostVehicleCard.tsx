...
export function GhostVehicleCard() {
  ...
  const fetchGhostVehicles = async () => {
    ...
    try {
      ...
      const oldVehiclesData = (oldVehicles || []) as unknown as { device_id: string; device_name: string | null; created_at: string }[];
      
      if (oldVehiclesData.length === 0) {
        setGhostVehicles([]);
        setLoading(false);
        return;
      }

      const deviceIds = oldVehiclesData.map((v) => v.device_id);
      ...
      const positionsData = (vehiclesWithPositions || []) as unknown as { device_id: string; latitude: number; longitude: number }[];
      const hasPositionSet = new Set(positionsData.map((v) => v.device_id));
