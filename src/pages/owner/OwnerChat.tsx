...
function VehicleChatItem({
  ...
  return <button onClick={onClick} className="w-full flex items-center gap-3 p-4 bg-card shadow-neumorphic-sm rounded-xl hover:shadow-neumorphic transition-all duration-200 active:shadow-neumorphic-inset text-left">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2">
          <h3 className="font-medium text-foreground text-sm truncate">
            {vehicle.name}
            {hasNickname && <span className="text-muted-foreground font-normal ml-1">
                ({vehicle.plateNumber})
              </span>}
          </h3>
      ...
      {vehicle.unreadCount > 0 ? <div className="shrink-0 min-w-[22px] h-[22px] px-1.5 rounded-full bg-accent shadow-[0_0_12px_hsl(24_95%_53%/0.4)] flex items-center justify-center">
          <span className="text-[10px] font-semibold text-accent-foreground">
            {vehicle.unreadCount}
          </span>
        </div> : <div className="w-8 h-8 rounded-full shadow-neumorphic-sm bg-card flex items-center justify-center">
...
export default function OwnerChat() {
  ...
  const filteredVehicles = vehicles?.filter(v => v.name.toLowerCase().includes(searchQuery.toLowerCase())) || [];
  return <OwnerLayout>
      <div className="flex flex-col h-full">
        ...
        <div className="sticky top-0 z-10 bg-background/95 backdrop-blur-sm">
          <div className="px-4 pt-4 pb-3">
            ...
            <div className="relative">
              ...
              <Input placeholder="Search conversations..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-11 bg-card border-0 shadow-neumorphic-inset h-12 rounded-xl focus-visible:ring-accent/30 text-foreground placeholder:text-muted-foreground" />
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-auto px-4 pb-4">
          {isLoading ? <div className="space-y-3 pt-2">
              {[1, 2, 3].map(i => <Skeleton key={i} className="h-20 rounded-xl" />)}
            </div> : filteredVehicles.length === 0 ? <div className="flex flex-col items-center justify-center py-20 px-4">
              ...
              <p className="text-sm text-muted-foreground text-center">
                {searchQuery ? "No vehicles match your search" : "Your vehicle chats will appear here"}
              </p>
            </div> : <div className="space-y-3 pt-2">
              {filteredVehicles.map(vehicle => <VehicleChatItem key={vehicle.deviceId} vehicle={vehicle} onClick={() => navigate(`/owner/chat/${vehicle.deviceId}`)} />)}
            </div>}
        </div>
      </div>
    </OwnerLayout>;
