...
export function useAdminWallets() {
  ...
  const fetchStats = async () => {
    ...
    const typedData = (data || []) as { amount: number; type: string }[];
    const credits = typedData.filter((t: any) => t.type === "credit").reduce((sum: number, t: any) => sum + parseFloat(String(t.amount)), 0) || 0;
    const debits = typedData.filter((t: any) => t.type === "debit").reduce((sum: number, t: any) => sum + parseFloat(String(t.amount)), 0) || 0;

    setStats({
      totalRevenue: debits,
      totalCredits: credits,
      totalDebits: debits,
      transactionCount: typedData.length || 0,
    });
  };
