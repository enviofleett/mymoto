-- Create AI Training Scenarios Table
-- Run this if the table doesn't exist
-- This is a standalone version of: supabase/migrations/20260114000006_create_ai_training_scenarios.sql

-- Check if update_app_settings_updated_at function exists, create if not
...
-- Insert default scenarios (only if table is empty)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.ai_training_scenarios LIMIT 1) THEN
    ...
    RAISE NOTICE '✅ Inserted 5 default AI training scenarios';
  ELSE
    RAISE NOTICE '✅ AI training scenarios table already has data, skipping default inserts';
  END IF;
END $$;

-- Verification
SELECT 
  '✅ AI Training Scenarios table created successfully!' as status,
  COUNT(*) as total_scenarios,
  COUNT(*) FILTER (WHERE is_active = true) as active_scenarios
FROM public.ai_training_scenarios;
