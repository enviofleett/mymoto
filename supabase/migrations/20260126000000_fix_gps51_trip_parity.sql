...
-- Description: 
-- Recreates vehicle_trips as a VIEW with: 
-- 1. GPS51-style ignition-based trip detection 
-- 2. 3-minute idle timeout for trip splitting 
-- 3. Accurate distance using odometer deltas 
-- 
-- This migration is SAFE for Supabase CLI: 
-- - No DO blocks 
-- - No dollar-quoted functions 
-- - Fully idempotent 

-- =====================================================
-- 1. Safely drop vehicle_trips (table or view)
-- =====================================================

-- We use a DO block to handle the case where we don't know if it's a table or a view.
-- This prevents "is not a table" or "is not a view" errors.
DO '
BEGIN
  -- 1. Try to drop as a TABLE
  BEGIN
