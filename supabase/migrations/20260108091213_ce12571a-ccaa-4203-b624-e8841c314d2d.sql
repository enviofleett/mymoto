-- Fix the get_fleet_stats RPC function
-- Issues: 1) avg_speed should exclude extreme outliers (>200 km/h is likely GPS error)
--         2) overspeeding should only include vehicles with actual speed > 0
...
BEGIN
  ...
  -- Moving vehicles (speed > 0 and reasonable, < 200 km/h)
  ...
  -- Overspeeding vehicles (only those actually moving with reasonable speed)
  SELECT count(*) INTO overspeeding_count 
  FROM vehicle_positions 
  WHERE is_overspeeding = true AND speed > 0 AND speed < 300;
  ...
  -- Average speed (for moving vehicles, excluding GPS errors > 200 km/h)
  ...
  -- Overspeeding details (top 5, only vehicles actually moving)
  ...
  FROM (
    ...
    LEFT JOIN vehicles v ON v.device_id = vp.device_id
    LEFT JOIN vehicle_assignments va ON va.device_id = vp.device_id
    WHERE vp.is_overspeeding = true AND vp.speed > 0 AND vp.speed < 300
