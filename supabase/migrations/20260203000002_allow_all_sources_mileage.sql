-- Migration: Allow all trip sources in mileage stats
-- Description:
-- 1. Update get_vehicle_mileage_stats to remove source restriction (allow position_history, null, etc.)
-- 2. Update get_daily_mileage to remove source restriction
-- 3. Update vehicle_daily_stats view to remove source restriction
--
-- This ensures that legacy data (null source) and position_history trips are counted in stats.
...
BEGIN
  ...
  -- REMOVED source filter to include ALL valid trips (legacy, position_history, etc.)
  SELECT
  ...
  WHERE t.device_id = p_device_id
    -- AND t.source IN ('gps51', 'fallback_calculation') -- Removed restriction
...
DROP FUNCTION IF EXISTS public.get_vehicle_daily_stats(TEXT, INTEGER);
...
WHERE start_time >= NOW() - INTERVAL '90 days'
  -- AND source IN ('gps51', 'fallback_calculation') -- Removed restriction
