-- Migration: Create Optimized Vehicle Trips Function
-- Description: 
-- Creates a performance-optimized RPC function get_vehicle_trips_optimized that
-- replaces the slow vehicle_trips view query. It forces filtering by device_id
-- at the earliest possible stage to utilize database indexes effectively.
...
BEGIN
  ...
  WITH vehicle_telematics_data AS (
    ...
    WHERE ph.device_id = p_device_id -- CRITICAL OPTIMIZATION: Push down filter
