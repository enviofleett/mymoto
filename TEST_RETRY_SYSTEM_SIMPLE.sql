-- =====================================================
-- SIMPLE TEST: Retry System
-- This version doesn't use the description column
-- =====================================================
...
BEGIN
  ...
  -- Create test event (without description column)
...
-- Check if notified column exists, then query accordingly
DO $$
BEGIN
  IF EXISTS (
    ...
    RAISE NOTICE '✅ notified column exists';
  ELSE
    -- Column doesn't exist
    RAISE NOTICE '⚠️ notified column does not exist - checking by chat messages only';
  END IF;
END $$;

-- Query that works with or without notified column
SELECT 
  e.id as event_id,
  e.device_id,
  e.event_type,
...
-- STEP 5: Summary
-- STEP 5: Summary (handles missing notified column)
SELECT 
  ...
  CASE 
    WHEN EXISTS (
      ...
      (SELECT COUNT(*) FROM proactive_vehicle_events WHERE metadata->>'test' = 'true' AND notified = true)
    ELSE
      (SELECT COUNT(*) FROM vehicle_chat_history WHERE is_proactive = true AND alert_id IN (SELECT id FROM proactive_vehicle_events WHERE metadata->>'test' = 'true'))
  END as events_notified,
